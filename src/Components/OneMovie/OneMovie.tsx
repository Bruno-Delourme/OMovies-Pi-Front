...
function OneMovie({ id, title, poster_path, overview, name, genre_ids, release_date, vote_average }: Partial<Movie>) {
  ...
  const isFavorite =
    favoriteMovies.find((movie) => movie.id === id) !== undefined;
  const isToReview =
    moviesToReview.find((movie) => movie.id === id) !== undefined;
  ...
  const handlePointerLeave = () => {
    if (!liked) {
      // Si le film n'est pas liké, désactive l'animation
  ...
  const containerClass =
    animate || liked ? "onemoviecontainer animate" : "onemoviecontainer";
  const likeButtonClass = liked ? "like-button liked" : "like-button";
  const plusButtonClass = plusAnimation
    ? "plus-button animated"
    : "plus-button";
          ...
          <div className="action-buttons min-full">
            ...
            {isToReview ? (
              <Button
                ...
                onClick={handleDeleteFromReview}
              >
